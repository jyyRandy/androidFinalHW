...
class News2Fragment : Fragment() {
    ...
    private val recyclerViewNews2: RecyclerView by lazy { requireView().findViewById(R.id.recyclerView_news_2) }  //要加requireView()
        ...
        getRetrofit().create(NewsService::class.java)
            ...
            .enqueue(object : Callback<NewsBean> {
                override fun onResponse(call: Call<NewsBean>, response: Response<NewsBean>) {
                    Log.d("wdw", "getNews success")
                    val newsBean = response.body()
                    val newsList = newsBean!!.news
                    ...
                    recyclerViewNews2.layoutManager = LinearLayoutManager(mContext)
                    recyclerViewNews2.adapter = News2FragmentAdapter(mContext, newsListTemp)
                }
                override fun onFailure(call: Call<NewsBean>, t: Throwable) {
                    Log.d("wdw", "getNews failed -> $t")
                }
            })
    }
