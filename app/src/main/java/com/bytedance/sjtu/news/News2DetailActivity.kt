...
class News2DetailActivity : AppCompatActivity() {
    ...
    private val bgCommentItem: TextView by lazy { findViewById(R.id.bg_comment_item) }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        editComment.addTextChangedListener(object : TextWatcher {  //监听输入框的输入状态
            ...
            override fun afterTextChanged(p0: Editable?) {
                if (editComment.text.isNotEmpty()) {  //输入框不为空，发布字体变红色
                    postComment.setTextColor(resources.getColor(R.color.silver_red))
                } else {
                    postComment.setTextColor(resources.getColor(R.color.light_gray))
                }
            }
        })
    ...
    private fun postCommentAnimator() {//TODO()
        Log.d("wdw", "postCommentAnimator")
        val animator1 = ObjectAnimator.ofFloat(bgCommentItem, "alpha", 0f, 1f, 0f)
//        val animator2 = ObjectAnimator.ofFloat(bgCommentItem, "alpha", 0f, 1f, 0f)
        val animSet = AnimatorSet()
        animSet.duration = 1500
        animSet.play(animator1)//.after(animator1)
        animSet.start()
    }
        ...
        getRetrofit().create(NewsService::class.java)
            ...
            .enqueue(object : Callback<CommentBean> {
                override fun onResponse(call: Call<CommentBean>, response: Response<CommentBean>) {
                    Log.d("wdw", "getComment success")
                    commentList = response.body()!!.comments.asReversed()
                    recyclerView.layoutManager = LinearLayoutManager(this@News2DetailActivity)
                    recyclerView.adapter = News2DetailAdapter(this@News2DetailActivity, commentList)
                    commentCount1.text = commentList.size.toString()
                    commentCount2.text = commentList.size.toString()
                    Log.d("wdw", "commentList.size = ${commentList.size}")
                }
                override fun onFailure(call: Call<CommentBean>, t: Throwable) {
                    Log.d("wdw", "getComment failed -> $t")
                }
            })
    }
    ...
    private fun postComment(comment: String) {
        getRetrofit().create(NewsService::class.java)
            .postComment(PostCommentInfo("121110910068", "wdw", comment))
            .enqueue(object : Callback<postCommentBean> {
                override fun onResponse(call: Call<postCommentBean>, response: Response<postCommentBean>) {
                    Log.d("wdw", "postComment success")
                    Toast.makeText(this@News2DetailActivity, "发布评论成功~", Toast.LENGTH_SHORT).show()
                }
